...
read_matrix_from_excel <- function(file_path, sheet = 1) {
  ...
  if (any(!is.na(as.numeric(row_names)) == FALSE)) {
    stop("Error: Row names (first column) contain non-numeric content.
         Please make sure that row names in the Excel file are all numeric.")
  }

  numeric_data <- as.data.frame(sapply(numeric_data, as.numeric))
  if (any(is.na(numeric_data))) {
    stop("Error: The numeric column contains non-numeric content.
         Please make sure all numeric data is digitized in the Excel file.")
  }
